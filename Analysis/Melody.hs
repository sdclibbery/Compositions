...
analyse = Prelude.concat . map analysePart . extractParts
  where
    analysePart = catMaybes . mapZipper ruleH89 . zipper
...
zipper s =  (toList $ mapWithSpan (\sp n -> sp =: n) s, [])
...
mapZipper f z = if isNothing n then [] else (f z) : (mapZipper f $ fromJust n) -- Can be written much better!
    where
        n = next z

instance HasGetPitch (ChordT BasicPitch) where
    __getPitch c = head $ getChord c
